...
public class Carro {
    ...
    private Carro(Motorista motorista, String placa, Cor cor) {
        if (placa == null) throw new NullPointerException();
        if (cor == null) throw new NullPointerException();
        if (motorista == null) throw new EstacionamentoException("Veículo precisa ser conduzido por um motorista");
        if (motorista.getIdade() < 18) throw new EstacionamentoException("Motorista menor de idade");
        if (motorista.getPontos() > 20) throw new EstacionamentoException("Motorista com excesso de pontos");
